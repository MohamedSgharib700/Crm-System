    ...
    <div class="card">
      ...
      <div class="card-body pt-0">
        ...
        >
          ...
          <template v-slot:cell-user_type="{ row: customer }">
            {{ customer.user_type }}
          </template>
          ...
          <template v-slot:cell-active="{ row: customer }">
            <div class="form-check form-switch">
              ...
              <input
                ...
                @change="is_Active(customer.id, $event)"
              />
...
import { IManager } from "@/store/modules/ManagerModule";
...
export default defineComponent({
  ...
  setup() {
    ...
    let list = computed(() => store.getters.getManager);
    let viewLink = (id) => {
      return "/Employees/" + id;
    };
    ...
    const tableData = ref<Array<IManager>>(list.value);
    const initCustomers = ref<Array<IManager>>([]);
    onMounted(() => {
        ...
        if (result.isConfirmed) {
          ...
          try {
            ...
            checkedCustomers.value.forEach((item) => {
              ...
              store.dispatch(Actions.DELETE_MANAGER, item);
    ...
    const deleteCustomer = (id) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          loading.value = true;
          try {
            store.dispatch(Actions.DELETE_MANAGER, id);
            for (let i = 0; i < tableData.value.length; i++) {
    ...
    const searchItems = () => {
      ...
      if (search.value !== "") {
        let results: Array<IManager> = [];
        for (let j = 0; j < tableData.value.length; j++) {
          let list = reactive({
            name: tableData.value[j].name,
            email: tableData.value[j].email,
            user_type: tableData.value[j].user_type,
          });
    ...
    const onEnlargeText = () => {
      ...
      error.value = store.getters.geterrorManager;
      ...
      if (error.value.length == 0) {
        Process_success.value = true;
        let data = store.getters.getdata_Add_Manager;
    ...
    const is_Active = (id, event) => {
      ...
      for (let i = 0; i < tableData.value.length; i++) {
        if (tableData.value[i].id == id) {
          checkedNames = {
            ...
            email: tableData.value[i].email,
            created_at: tableData.value[i].created_at,
            user_type: tableData.value[i].user_type,
      ...
      if (index !== -1) tableData.value.splice(index, 1, checkedNames);

      console.log("checkedNames_on = ", checkedNames);
      const data = {
        is_active: event.target.checked,
      };
