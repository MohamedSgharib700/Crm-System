        ...
        <div class="card-title">
          ...
          <div class="d-flex align-items-center position-relative my-1">
            ...
            <input
              ...
              placeholder="بحث عن مقدمين الخدمات"
            />
          </div>
          <!--end::Search-->
        </div>
...
import { IServices } from "@/store/modules/ServicesModule";
...
export default defineComponent({
  ...
  setup() {
    ...
    const file_empty = ref<boolean>(false);
    ...
    let list = computed(() => store.getters.GetServices);
    let viewLink = (id) => {
      return "/services/" + id;
    };
    const tableData = ref<Array<IServices>>(list.value);
    const initCustomers = ref<Array<IServices>>([]);
    onMounted(() => {
      MenuComponent.reinitialization();
      setCurrentPageBreadcrumbs("مقدمين الخدمات", [
        "/services",
        "قائمه مقدمين الخدمات",
      ]);
   ...
   const deleteFewCustomers = () => {
        ...
        if (result.isConfirmed) {
          ...
          try {
            ...
            checkedCustomers.value.forEach((item) => {
              ...
              store.dispatch(Actions.Delete_Services, item);
    ...
    const deleteCustomer = (id) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          loading.value = true;
          try {
            store.dispatch(Actions.Delete_Services, id);
            for (let i = 0; i < tableData.value.length; i++) {
            ...
            loading.value = false;
            error.value = store.getters.getErrorServices;
            if (error.value.length !== 0) {
              console.log("error.value.length", error.value.length);
              error_show.value = true;
            }
    ...
    const onEnlargeText = () => {
      ...
      if (error.value.length == 0) {
        Process_success.value = true;
        let data = store.getters.getdata_Add_services;
